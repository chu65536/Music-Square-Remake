...
Game::Game() {
    m_window.create(sf::VideoMode(m_userData.windowSize.x, m_userData.windowSize.y), "Title", sf::Style::Fullscreen);
...
void Game::update() {
    ...
    if (m_userData.debugWindow) {
        debugWindow(m_gameData, m_userData, deltaTime);
    }
...
void Game::render() {
    m_window.clear(m_userData.wallsColor); 
...
void Game::handleEvents() {
    ...
    while(m_window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::KeyPressed) {
            ...
            switch(key) {
            case sf::Keyboard::D:
                m_userData.debugWindow = !m_userData.debugWindow;
                break; 
            }
        }
...
void Game::setState(State::Type type) {  
    ...
    case State::Type::Settings:
        m_currentState = std::make_unique<Settings>(m_userData);
        break;
    case State::Type::Load:
        m_currentState = std::make_unique<Load>(m_gameData, m_userData);
        break;
    case State::Type::Play:
        m_currentState = std::make_unique<Play>(m_gameData, m_userData);
        break;
    }
}

void Game::debugWindow(const GameData& gameData, const UserData& configData, const sf::Time& dt) const {
    int location = 0;
