...
void Map::Init(const std::vector<double>& delays, const UserData* configData, Square* square, std::mutex& m) {
    ...
    while (currentSize != delays.size()) {
        {
            ...
            if (collision) {
                while(currentSize != 1 && platforms_[currentSize - 1].TryAnotherDirection() == false) {
                    platforms_.pop_back();
...
void Map::addBackground() {
    ...
    back.setFillColor(configDataPt_->backgroundColor); 
